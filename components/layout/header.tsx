...
export function Header() {
  ...
  return (
    <header className="h-[64px] fixed top-0 right-0 left-0 md:left-[256px] z-30 bg-[rgba(0,0,0,0.8)] backdrop-blur-xl border-b border-[rgba(255,255,255,0.06)] flex items-center justify-between px-6 transition-all duration-200">
      ...
      {/* suppressHydrationWarning here because isAuthenticated differs between
          server (always false, no Supabase session) and client (reads real session).
          React will still render the correct UI on the client after hydration. */}
              ...
              {showDropdown && (
                <>
                  ...
                  <div className="absolute top-12 right-0 w-48 bg-[#1a1a1a] border border-[rgba(255,255,255,0.08)] rounded-xl py-2 shadow-2xl z-50">
                    ...
                    <Link href="/settings" onClick={() => setShowDropdown(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-[#a1a1aa] hover:bg-[#222222] hover:text-white transition-colors">
                    ...
                    <button onClick={() => { signOut(); setShowDropdown(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[#a1a1aa] hover:bg-[#222222] hover:text-white transition-colors">
